  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (form.scope === "national") {
        payload.states = ["ALL"]; payload.cities = []; payload.zipCodes = [];
      } else if (form.scope === "state") {
        payload.states = parseArray(form.states); payload.cities = []; payload.zipCodes = [];
        if (!(payload.states as string[]).length) { toast.error("Enter at least one state"); setSaving(false); return; }
      } else if (form.scope === "city") {
        payload.states = parseArray(form.states); payload.cities = parseArray(form.cities); payload.zipCodes = [];
        if (!(payload.cities as string[]).length) { toast.error("Enter at least one city"); setSaving(false); return; }
      } else {
        payload.states = parseArray(form.states); payload.cities = parseArray(form.cities); payload.zipCodes = parseArray(form.zipCodes);
      }
      ...
      if (data.success) {
        ...
        setShowForm(false); setEditingId(null); setForm(emptyForm); fetchTerritories();
